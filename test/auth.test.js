...
test("login repairs a modern policy that overrides the legacy sub2api wildcard", () => {
  const config = { agents: { defaults: {
    models: { "sub2api-provider/*": { alias: "sub2api" } },
  ...
  assert.deepEqual(patch.defaults.models["sub2api-provider/*"], { alias: "sub2api" });
...
test("an explicitly empty policy gains only sub2api access", () => {
